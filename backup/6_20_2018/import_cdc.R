...
datvars = c('mort.ucd.drugalc','mort.ucd.suicide','mort.ucd.all')
...
# Main Loop
varlist = c('mort.ucd.drugalc','mort.ucd.opioid','mort.ucd.suicide','mort.ucd.all','pop')
for (var in varlist) {
...
varlist = c('mort.ucd.drugalc','mort.ucd.opioid','mort.ucd.suicide')
suffixlist = c('.12','.8','.disc95')
for (var in varlist) {
   for (suffix in suffixlist) {
      cdc.mort.cnty[, paste(var,suffix,'.pc',sep=''):= get(paste(var,suffix,sep=''))/get(paste('pop',suffix,sep=''))]
      cdc.mort.cnty[, paste(var,suffix,'.pd',sep=''):= get(paste(var,suffix,sep=''))/get(paste('mort.ucd.all',suffix,sep=''))]  
   }
}
