...
for (i in 1:length(keithperms)) {
    yhats$rf[[keithperms[i]]] = data.table(keithdata[, i + 1, with = F], weight = keithdata$cc.commonweight_vv_post, statename = keithdata$statename)
...
for (elec in names(st.votepost)) {
    ...
    st.votepost[[elec]]$dat$winner = mapvalues(st.votepost[[elec]]$dat$winner, c(1, 2, 3), c('gop', 'dem', 'oth'))
