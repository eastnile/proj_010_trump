...
for (var in getReg(1.1, regdir.good)$v.y) {
    for (margtype in c('mam')) {
        ...
        #margtype = 'mam'
        t = summary(modcces$logit.fscaled[[margtype]][[var]])[, c('factor', 'AME', 'p')]
        t$absAME = abs(t$AME)
        t$info = paste(var, margtype)
        horserace$cces[[margtype]][[var]] = merge(regdir[, .(varname.raw, varname.print, table.cat)], t,
...
# Glue together, CCES:
sheetout = horserace$cces[[1]][[1]][0,]
for (var in getReg(1.1, regdir.good)$v.y) {
    for (margtype in c('mam', 'ame')) {
        #var = getReg(2.1, regdir.good)$v.y[2]
        #margtype = 'mam'
        t = horserace$cces[[margtype]][[var]]
...
# Write variables to list, for easy handling
