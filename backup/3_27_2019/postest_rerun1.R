...
load(file = 'C:/Users/immanuel/Google Drive/research/proj_010_trump/backup/3_27_2019/stvote.Rdata')
#load(file = 'regdata.Rdata')
load(file = 'elecvotes.Rdata')

# Functions
ge.predict = function(mod, ccesdat.mlogit, name) { # function to predict results
    #prefix = 'test1'
    ...
    returnobj = list()
    returnobj$stvote = select(pred.st.vote, statename, contains('.pc'))
    returnobj$yhatdt1 = z.predict
    returnobj$yhatdt2 = postest.cces
    return(returnobj)
}
...
ccesperm$e9_crashtest = copy(ccesplus.small)
ccesperm$e9_crashtest[, crashpc := 100]
...
for (elec in names(st.vote)) {
    ...
    st.vote[[elec]]$tally = st.vote[[elec]]$dat[, sum(evotes), by = winner]
...
for (elec in names(st.vote)) {
    print(elec)
    elecplots[[elec]] = plotelec(plotdata = st.vote[[elec]]$dat, savename = elec,
                                    plot1.title = paste(elec, st.vote[[elec]]$tallystr),
                                    plot2.title = paste(elec, 'GOP margin of victory'),
                                    plot1.legend = st.vote[[elec]]$tallystr,
                                    plot2.legend = st.vote[[elec]]$tallystr)
}
